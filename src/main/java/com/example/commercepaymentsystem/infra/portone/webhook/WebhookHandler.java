...
import com.example.commercepaymentsystem.infra.portone.webhook.entity.WebhookEvent;
import com.example.commercepaymentsystem.infra.portone.webhook.service.WebhookEventService;
...
public class WebhookHandler {
    ...
    private void handlePaid(Long eventId, String portonePaymentId) {
        ...
        Long dbPgAmount = payment.getOrder().getPgAmount();

        if (!Objects.equals(dbPgAmount, pg.totalAmount())) {
            webhookEventService.markFailed(
                    eventId,
                    "금액 불일치: db=" + dbPgAmount + ", pg=" + pg.totalAmount()
            );
            return;
        }
        ...
        if (payment.getStatus() == PaymentStatus.PENDING) {
            paymentCommandService.processPaidWebhook(portonePaymentId);
        }
    ...
    private void handleCancel(Long eventId, String portonePaymentId) {
        ...
        webhookEventService.markIgnored(eventId, "취소 웹훅은 현재 자동 반영하지 않음: " + portonePaymentId);
    }
